...
# Taken from the MariaDB official build, with some changes to remove unnecessary
# users, and change some directory ownership so the container can be run as a
# non-root user (check out the OpenShift docker containers -- google it -- for
# other prototypes to implement
...
# Fix permissions to allow for running as non-root user (borrowed from openshift)
...
RUN chmod a+x ./fix-permissions.sh && \
    ...
    ./fix-permissions.sh /var/log/mysql.err && \
    ./fix-permissions.sh /var/log/mysql.log
